...
class BackgroundObserverTests: ProcedureKitTestCase {
    ...
    // MARK: Cancellation Behavior: .whenBackgroundExecutionTimeExpires

    func test__background_observer__cancel_when_background_time_expires__app_already_in_background() {

        let expectedBackgroundTaskName = BackgroundManager.backgroundTaskName(for: backgroundProcedure)
        testableApplication.enterBackground()

        observer = BackgroundObserver(manager: testableBackgroundManager, cancelProcedure: .whenBackgroundExecutionTimeExpires)
        ...
        XCTAssertEqual(didCancel.wait(timeout: .now() + 0.3), .timedOut, "The Procedure was cancelled prior to the background execution time expiring.")

        // simulate the background execution time expiring
        testableApplication.simulateBackgroundTimeExpiration()

        // wait for the expected Procedure cancellation
        ...
        XCTAssertTrue(receivedCancellationErrors[0] is ProcedureKitError.AppBackgroundTimeExpired)

        XCTAssertEqual(testableApplication.backgroundTasks.count, 1)
        XCTAssertEqual(testableApplication.backgroundTasks[0].0, expectedBackgroundTaskName)
    }

    func test__background_observer__cancel_when_background_time_expires__app_transitions_to_background_while_executing() {
        ...
        didCancel.enter()
        backgroundProcedure.addDidExecuteBlockObserver(synchronizedWith: DispatchQueue.main) { [testableApplication = testableApplication!] backgroundProcedure in
            // enter the background while executing
            testableApplication.enterBackground()

            // verify that the Procedure isn't cancelled (yet)
            XCTAssertEqual(didCancel.wait(timeout: .now() + 0.3), .timedOut, "The Procedure was cancelled prior to the background execution time expiring.")

            // simulate the background execution time expiring while executing
            testableApplication.simulateBackgroundTimeExpiration()
        }
        backgroundProcedure.addDidCancelBlockObserver { backgroundProcedure, errors in
            cancellationErrors.append(contentsOf: errors)
            didCancel.leave()
            backgroundProcedure.finish() // finish when cancelled
        }
        ...
        // run and wait for the Procedure
        wait(for: backgroundProcedure)
        // and all background tasks
        waitForTaskGroup()

        // wait for the expected Procedure cancellation
        XCTAssertEqual(didCancel.wait(timeout: .now()), .success, "The Procedure was not cancelled.")
        ...
        XCTAssertProcedureCancelledWithErrors(backgroundProcedure)
    }
