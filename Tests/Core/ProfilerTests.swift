...
class PrintableProfileResultTests: XCTestCase {
    ...
    func test__description__with_children() {
        ...
        let child1 = ProfileResult(identity: OperationIdentity(identifier: "Child 1", name: "Data Procedure"), created: now + 0.2, attached: 0.1, started: 0.2, cancelled: .none, finished: 0.3, children: [child2])
        ...
        printable = PrintableProfileResult(result: result)
        XCTAssertEqual(printable.description, "+0.1 Attached\n+0.2 Started\n-> Spawned Data Procedure #Child 1 with profile results\n  +0.1 Attached\n  +0.2 Started\n  -> Spawned Unnamed Procedure #Child 2 with profile results\n    +0.1 Attached\n    +0.2 Started\n    +0.3 Finished\n  +0.3 Finished\n+0.3 Finished\n")
    }
}
