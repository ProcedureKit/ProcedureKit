...
class DeviceReachability: NetworkReachabilityType {
    ...
    func startNotifierOnQueue(_ queue: DispatchQueue) throws {
        ...
        if !notifierIsRunning {
            ...
            /// HACK to work around toOpaque() not existng in Swift 3.0 preview branch
            // Befor: Unmanaged.passUnretained(self).toOpaque()
            let __opaquePointerToSelf = unsafeBitCast(self, to: UnsafeMutablePointer<Void>.self)
            context.info = __opaquePointerToSelf
