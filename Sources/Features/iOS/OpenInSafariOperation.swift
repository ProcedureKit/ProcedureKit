...
public class OpenInSafariOperation<From: PresentingViewController>: GroupOperation {
    ...
    private var displayControllerFrom: ViewControllerDisplayStyle<From>?
     ...
     */
    public init(URL: NSURL, displayControllerFrom from: ViewControllerDisplayStyle<From>? = .None, entersReaderIfAvailable: Bool = false, sender: AnyObject? = .None) {
        self.URL = URL
        ...
        if errors.isEmpty && !cancelled, let decision = operation as? DecideWhereToOpenOperation {
            if decision.shouldOpenInSafariViewController {
                if #available(iOS 9.0, *) {
                    let from = displayControllerFrom ?? ViewControllerDisplayStyle.Show(UIApplication.sharedApplication().keyWindow!.rootViewController! as! From)
                    
                    produceOperation(WebpageOperation(url: URL, displayControllerFrom: from, sender: sender))
                }
            } else {
